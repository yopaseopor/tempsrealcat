...
function develop_parser(element) {
    ...
    markerPopup += Mustache.render(
	'<a href="{{link}}" target="_blank">Visualiza en OpenStreetMap</a> <br />',
	{link: link}
    );
    ...
    markerPopup += Mustache.render(
	'<a href="{{link}}" target="_blank">Edita en OpenStreetMap</a> <br />',
	{link: link}
    );
...
function website_parser(element) {
	...
	markerPopup += Mustache.render(
	    tagTmpl,
	    {tag: 'Web', value: link, iconName: 'globe'}
	);
    }
    return markerPopup;
}
...
function email_parser(element) {
	...
	markerPopup += Mustache.render(
	    tagTmpl,
	    {tag: 'Correo', value: link, iconName: 'at'}
	);
    }
    return markerPopup;
}
...
function phone_parser(element) {
	...
	markerPopup += Mustache.render(
	    tagTmpl,
	    {tag: 'Teléfono', value: link, iconName: 'phone'}
	);
    }
    return markerPopup;
}
...
function opening_tag_parser(element) {
	...
	markerPopup += Mustache.render(
	    tagTmpl,
	    {tag: 'Horario', value: tags['opening_hours'], iconName: 'clock-o'}
	);
    }
    return markerPopup;
}
...
function parse_osm_times(element) {
	...
	var nextchange = oh.getNextChange();
        // console.log(state)
	if (state == "open") {
	return "<span style='color:green' class='fa fa-circle'></span> <b>Estado:</b> Abierto.<br><span style='color:red' class='fa fa-arrow-circle-down'></span> <b>Cerrarà:</b> " + nextchange + "<br>";
	}
	else if (state == "close") {
	return "<span style='color:red' class='fa fa-circle'></span> <b>Estado:</b> Cerrado.<br><span style='color:green' class='fa fa-arrow-circle-up'></span> <b>Abrirá:</b> " + nextchange + "<br>";
	}
	else if (state == "undefined") {
	return "<span style='color:gray' class='fa fa-circle'></span> <b>Estado:</b> Indefinido.<br><span style='color:gray' class='fa fa-arrow-circle-right'></span> <b>Cambiará su estado:</b> " + nextchange + "<br>";
	}
    ...
    catch(err)
    {
        ...
        return "<span style='color:gray' class='fa fa-circle'></span> <b>Estado:</b> Desconocido<br>";
    }
}
...
function parse_tags(element, titlePopup, functions) {
	...
	{callback: generic_tag_parser, tag: 'description', label: 'Descripción'},
    ].concat(functions)
	...
	markerPopup += '<br><a href="#" onclick="javascript: sidebar.open(\'developer\'); return false;">Información al detalle (experto)</a>';
